...
function InTheatres({ data }) {
  if (data && data.length > 4) data = data.slice(0, 4);
                  ...
                  <div className='df fww just-between'>
                    <figure className='pvr'>
                      <Link href={item.link.replace(process.env.NEXT_PUBLIC_BACKEND_URL, '')}>
                        ...
                        <Image src={item.img.url} width='500' height='500' alt={item?.img?.alt || 'Now in Theatres Movies'} className='objctimg_box' />
                      </Link>
                    </figure>
                    <div className='homtheaterbox_txt'>
                      ...
                      <p title={item.genre} className='line-clamp-2'>{item.genre}</p>{' '}
                    </div>
                  </div>
                </div>
              ) : null}
