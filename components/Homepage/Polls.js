...
import $ from 'jquery';
// import charimg from '@/public/barchart.webp';
...
function Polls({ data, tag, ip }) {
  ...
  return (
    <div className='ansoptionbox df fww xl:min-h-[550px] xl:-mt-[228px]'>
      {data &&
        data.map((item, id) => {
            ...
            <div id={div_id} className='ansoptionbox_in df fww' style={{ display: id === 0 ? 'block' : 'none' }} key={poll_id}>
              ...
              {submitted === undefined || submitted === null ? (
                <div className={div_cls}>
                  ...
                  {item.image && (
                    <div className='pollitemimg mt-3'>
                ...
                <div className={result_cls}>
                  ...
                  <ul className='resultbox'>
                    {item.options.map((option, index) => {
                      ...
                      let options = '';
                      if (tag === 'poll' && item.poll_results === undefined) options = '0';
                      if (tag === 'quiz' && item.quiz_result === undefined) options = '0';
                      if (options !== '0') {
                        options = tag === 'poll' ? item.poll_results && item.poll_results[optionHTML] : item.quiz_result && item.quiz_result[optionHTML].results;
                      }
