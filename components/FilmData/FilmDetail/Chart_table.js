...
const Charttable = ({ data, charton, isReRelease, ReleaseDate }) => {
  isReRelease = isReRelease || false;
  useEffect(() => {
    ...
    let basemovie = isReRelease ? data.release_name : $$('.moviename .chartlegend:first-child').dataset.text;
    if ($$$('.moviename .chartlegend').length > 1 && $$$('.chartdatafeed > div').length > 1 && !isReRelease) {
    ...
    let boxoptions = {
      ...
      tooltip: {
        ...
        custom: function ({ series, seriesIndex, dataPointIndex, w }) {
          ...
          let seriesinfo_three,
            seriousthreetotaltip,
            toolthree = '';
          if (srthree > 0) {
          ...
          if (makeDataonebase[dataPointIndex] > 0) {
            toolone =
              ...
              '<div>' +
              'Weekly (7-Day): $' +
              seriesinfo_one.data[dataPointIndex].goals[seriesIndex].strokeWidth +
              'M' +
              '</div>' +
              '</div>';
          }
          ...
          if (srtwo > 0) {
            tooltwo =
              ...
              '<div>' +
              'Total To Date: $' +
              seriesinfo_two.data[dataPointIndex].y +
              'M' +
              '</div>' +
              '<div>' +
              'Weekend (3-Day): $' +
              seriesinfo_two.data[dataPointIndex].goals[seriesIndex].value +
              'M' +
              '</div>' +
              '<div>' +
              'Weekly (7-Day): $' +
              seriesinfo_two.data[dataPointIndex].goals[seriesIndex].strokeWidth +
              'M' +
              '</div>' +
              '</div>';
          }
          ...
          if (srthree > 0) {
            toolthree =
              ...
              "' ></div>" +
              "<h5 class='series_name'>" +
              seriesinfo_three.name +
              '</h5>' +
              '<div>' +
              'Total To Date: $' +
              seriesinfo_three.data[dataPointIndex].y +
              'M' +
              '</div>' +
              '<div>' +
              'Weekend (3-Day): $' +
              seriesinfo_three.data[dataPointIndex].goals[seriesIndex].value +
              'M' +
              '</div>' +
              '<div>' +
              'Weekly (7-Day): $' +
              seriesinfo_three.data[dataPointIndex].goals[seriesIndex].strokeWidth +
              'M' +
              '</div>' +
              '</div>';
          }
          ...
          tooltipbox = '<div class="charttool_box">' + "<div class='week_name'>" + w.globals.labels[dataPointIndex] + '</div>' + "<div class='series_infotool'>" + toolone + tooltwo + toolthree + '</div>' + '</div>';
            ...
            <div className='chartdatafeed'>
              {data.chart.map((items, index) => {
                if (index == 0) {
                  ...
                  return <div className={index == 1 ? 'seriesthree' : 'seriesfour'} data-weekend={'[' + items.w_end + ']'} data-weekly={'[' + items.w_ly + ']'} data-total={'[' + items.tot_ly + ']'} key={index}></div>;
                }
              })}
            </div>
            <div className='comparechart_box pvr'>
              <div className='milian'>($1m)</div>
              <div id='boxchart' key={ReleaseDate}></div>
            </div>
          </div>
        </div>
      </section>
