...
const TheatreTiming = ({ data, film_id, mdetailshow }) => {
  ...
  useEffect(() => {
    if (userLocationData !== '' && userLocationData.length > 4) loadShowTimeZipData();
  }, [userLocationData]);

  const loadShowTimeData = async () => {
    setLoaderTheatreZip(LOADER);
    let showTimeAPI =
      process.env.NEXT_PUBLIC_SD_API + '/show_time/FilmDetailShowtime.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'film-detail/' + film_id + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN;
    if (userlatitude !== undefined && userlatitude !== null) showTimeAPI += '&latitude=' + userlatitude;
    if (userlongitude !== undefined && userlongitude !== null) showTimeAPI += '&longitude=' + userlongitude;
    if (userpincode !== undefined && userpincode !== null) showTimeAPI += '&pincode=' + userpincode;
    axios
      .get(showTimeAPI)
      .then((res) => {
        setLoaderTheatreZip('');
        setShowTimeData(res.data);
        res.data.showtime_list.sort((a, b) => (a.miles_distance < b.miles_distance ? -1 : 1));
    ...
    axios
      .get(
        process.env.NEXT_PUBLIC_SD_API +
          '/show_time/FilmDetailShowtime.php?url=' +
  ...
  const success = (pos) => {
    ...
    fetch(coordinatesUrl)
      ...
      .then((json) => {
        ...
        setUserLocationData(json.results[0].components.postcode);
    ...
    <>
      <section className='theater_timing toplinesec'>
        <div className='container'>
          {!mdetailshow && (data.format || data.film_country || data.film_language) && (
          ...
          {ShowTimeDataLoaded &&
            !mdetailshow &&
            (ShowTimeData.showtime_list.length >= 1 ? (
