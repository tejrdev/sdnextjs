...
const Describe = ({ children, active, movielover, industry, loader }) => {
  ...
  const getDescribe = async () => {
    var get_describe_url = API_URL + '/login/describe.php';
    await axios
      .get(get_describe_url, {
        params: {
    ...
    <>
      ...
      <div className={`prostep_item ${active}`}>
        ...
        <div className="discribebest_box grid">
          <div className={checkMovie === 'industry' ? 'decbestiem active' : 'decbestiem'}
          onClick={() => movieUpdate('industry')}>
            <div className="top_ico">
              <Image src={INDST}
               rel="preload" as="image" title="Industry" alt="Industry" />
            </div>
            <h6>Industry Professional</h6>
            <p>(For those who work with information about movies and related business matters.)</p>
          </div>
          <div className={checkMovie === 'movielover' ? 'decbestiem active' : 'decbestiem'}
          onClick={() => movieUpdate('movielover')}>
            <div className="top_ico">

              <Image src={POPMOVIE}
               rel="preload" as="image" title="pop movie" className="animation" alt="pop movie" />
            </div>
            <h6>Movie Lover</h6>
