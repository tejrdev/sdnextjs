...
const Reviews = ({ data, bgin = false }: { data: any[], bgin?: boolean }) => {
    // console.log(bgin);
    return (
        <section className={`reviews py-8 md:py-12 lg:py-14 ${bgin ? 'bg-zinc-100' : ''}`}>
            <div className="container">
                ...
                <div className="reviews_list flex flex-wrap gap-5">
                        ...
                        <div className="newsancer_item xl:flex-flex-[1_1_calc(50%-20px)] w-full lg:w-[calc(50%-20px)]" key={index}>
                            <div className="bg-white rounded-lg flex flex-col sm:flex-row items-start border border-gray-600 ">
                                ...
                                <div className="w-full sm:w-[calc(100%-150px)] xl:w-[calc(100%-180px)] py-2 px-3 flex flex-col justify-between">
                                    <div>
                                        <p className="text-sm text-gray-600 my-1 flex items-center">
                                            <span className='uppercase'>{article.published_date}</span>
                                            {article.read_time && <span className='ml-3 mb-1  text-sm flex items-center'>
