...
export const Postercta = ({ btn, data, download }) => {
  return (
    <div className={'rsitem text-center max-w-[180px] ' + (data.var ? data.var : '')}>
      <figure className=''>
        <span className='pvr'>
            ...
            <div className='issueitems pvr' key={index}>
              <Link href={item.link}>
                ...
                <div className='btncta'>
                  <span className='btn uppercase py-3 px-2'>
                    <span>{'view recent'}</span>
                    <HiArrowNarrowRight className="inline-block align-top" />
                  </span>
                </div>
              </Link>
            </div>
