...
import mapdata from '../../public/mapdataCanada.json';
...
function Canada_Chart({ chrtmapdata }) {
  let [caStates, setCAStates] = useState(null);
  ...
  if (!caStates) {
    Promise.all([fetch('https://raw.githubusercontent.com/Anujarya300/bubble_maps/master/data/geography-data/canada.topo.json').then((r) => r.json())]).then(([canada]) => {
      setCAStates(canada);
    });
  }
  ...
  if (!caStates) {
    ...
    return (
      <>
        <USPageMap data={chartData} caStates={caStates} />
      </>
    );
  }
}

const USPageMap = ({ data, caStates }) => {
  useEffect(() => {
    ...
    const states = ChartGeo.topojson.feature(caStates, caStates.objects.canada).features;
    /* console.log(caStates) */
