...
export async function getStaticProps() {
  ...
  let exhibitorsData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/directory_exhibitors/new_exhibitor_list.php?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN + '&sortby=screen');
  exhibitorsData = await exhibitorsData.json();

  return {
    props: { SEOdata, exhibitorsData },
...
const Exhibitors = (props: Props) => {
  const { SEOdata, exhibitorsData } = props;
  ...
  const [exhibitorTypes, setExhibitorTypes] = useState<string>('');
  ...
  const setExhibitorFilter = (exhibitorType: string) => {
  ...
  useEffect(() => {
    ...
    loadExhibitorsData();
  }, [exhibitorTypes, exhibitorPage, SearchClicked, DirectorySortBy]);
        ...
        <div className='container'>
          <div className='distlist_box'>
            <div className='distsponsor_box df fww'>
              ...
              <Filters setListingFilter={setExhibitorFilter} data={exhibitorsData.filter_options.split(',')} tag='exhibitor' stateValue={exhibitorTypes} onClearAllClick={onClearAllClick} />
