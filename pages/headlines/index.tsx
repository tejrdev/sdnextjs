...
interface HeadlinesProps {
    featureheroheadlines: HeroHeadlineItem[];
    headlines: HeadlineItem[];
    featureheadline: any[];
    featureheadlinesidebar: any[];
    [key: string]: any;
}

export const getStaticProps: GetStaticProps<HeadlinesProps> = async () => {
    //headline listing
    //let headlineinfo = await fetch('https://tejrdev.github.io/api/headlinedata.json');

    const headlineResponse = await fetch(process.env.NEXT_PUBLIC_SD_API + '/headlines?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);

    const headlineinfo: HeadlinesProps = await headlineResponse.json();
...
const Header = (props: HeadlinesProps) => {
    ...
    const [newsbox, setNewsbox] = useState<any[][]>(newsboxitems);
    const [count, setCount] = useState<number>(0);

    function handleMoreNews(e: React.MouseEvent<HTMLButtonElement>, index: number, secdata: HeadlineItem) {
    ...
    useEffect(() => {
        if (post_id && typeof window !== 'undefined') {
            axios
                .get(process.env.NEXT_PUBLIC_SD_API + '/headlines/get_shared_post.php/?post_id=' + post_id + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
                .then((res) => {
                    //ftrhero = res.data.featureheroheadlines;
                    if (res.data) {
                        const newsDataElement = document.getElementById('news_new_news_data');
                        if (newsDataElement) {
                            newsDataElement.innerHTML = res.data.data;
                        }
                        //console.log('sss');
                    }
                })
    ...
    function sd_magnificPopup() {
        ...
        (window.$('.termtxt') as any).magnificPopup({
            type: 'inline',
            preloader: false,
            focus: '#name',
            callbacks: {
                beforeOpen: function () {
                    if ((window.$(window) as any).width() < 700) {
                        (this as any).st.focus = false;
                    } else {
                        (this as any).st.focus = '#name';
                    }
                },
                open: function () { },
                close: function () { },
                                        ...
                                        {ftrheroinfo.popclass ? (
                                            <a href={'#newsheropop-' + index} className={'df fww ' + ftrheroinfo.popclass}>
                                                <figure className='pvr'>
                                                    <img src={ftrheroinfo.img} width='454' height='260' alt='' className='objctimg_box' />
                                                </figure>
                                            ...
                                            <>
                                                <figure className='pvr'>
                                                    <a href={ftrheroinfo.link} className={'df fww '} target='_blank' rel='noopener noreferrer'>
                                                        <img src={ftrheroinfo.img} width='454' height='260' alt='' className='objctimg_box' />
                                                    </a>
                                                </figure>
                                                ...
                                                <div className='newsline_iteminfo'>
                                                    ...
                                                    <div className='fmupdate_boxaurthor'>
                                                        <a href={ftrheroinfo.source_url} className='' target='_blank' rel='noopener noreferrer'>
                                                            <div className='bgimage' style={{ background: `url(${ftrheroinfo.source_icon})` }}></div>
                                                            <span className='mediasrc'>{ftrheroinfo.source}</span>
                                                        </a>
                                                        <span>&nbsp;&nbsp; {ftrheroinfo.date_time}</span>
                                                ...
                                                <div className='sdnewspop'>
                                                    <figure className='pvr'>
                                                        ...
                                                        <span className='srcdate'>
                                                            <span className='bgimage' style={{ background: `url(${ftrheroinfo.source_icon})` }}></span>
                                                            {ftrheroinfo.source} {','} {ftrheroinfo.date_time}{' '}
                                                        </span>
                                                    </figure>
                                                    <h4>{ftrheroinfo.title}</h4>
                                                    {/*<span dangerouslySetInnerHTML={createMarkup()}> </span> */}
                                                    <div className='newsinfopoptxt' dangerouslySetInnerHTML={{ __html: ftrheroinfo.popinfo || '' }}></div>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                    ...
                    <>
                        ...
                        <div className='headlinecategory' key={index} data-title={newsdata.title}>
                            <h3 className='newsline_datebox'>
                                {newsdata.title} <i className='far fa-angle-right'></i>
                            </h3>
                            <div className='newsheadline_block threecol_news df fww'>
                            ...
                            <div className='viewmorebtn text-center'>
                                <button
                                    className='ghostbtn goldbtn'
                                    onClick={function (e) {
                                        handleMoreNews(e, index, newsdata);
                                    }}>
                                    view more
                                </button>
                            </div>
