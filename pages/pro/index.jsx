...
export default function ScreendollarPro_Home({ Pro_data }) {
  ...
  useEffect(() => {
    ...
    if (!userLoggedIn && !user) {
      //if both not set >> normal user without login, do nothing
    } else if (!userLoggedIn && !user) {
      //both available>> login user
    } else if ((userLoggedIn && !user) || (!userLoggedIn && user)) {
      //localstorage available, session storage not set OR vise-a-versa >> redirect to login to set sessionstorage value
