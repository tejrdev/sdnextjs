...
export async function getStaticProps() {
  ...
  let HollywoodReportData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/hollywood-report/&page_no=' + 1 + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const HollywoodReport = ({ data, HollywoodReportData }) => {
  ...
  const loadHollywoodReportData = () => {
    axios
      .get(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/hollywood-report/&page_no=' + HollywoodReportPage + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
      .then((res) => {
        ...
        <section className='sidebar_blockarea'>
          <div className='container'>
            <div className='sidebar_blockiner df fww'>
              ...
              <Info data={HollywoodReportListData} setCurrentPage={setCurrentPage} totalPages={HollywoodReportPages} currPage={HollywoodReportPage} />
            </div>
          </div>
        </section>
      </>
