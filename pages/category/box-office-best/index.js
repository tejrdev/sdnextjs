...
const BoxOfficeBest = ({ data }) => {
  ...
  const loadBoxOfficeBestData = () => {
    axios
      .get(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/box-office-best/&page_no=' + BoxOfficeBestPage + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
      .then((res) => {
          ...
          <section className='sidebar_blockarea'>
            <div className='container'>
              <div className='sidebar_blockiner df fww'>
                ...
                <Info data={BoxOfficeBestListData} setCurrentPage={setCurrentPage} totalPages={BoxOfficeBestPages} currPage={BoxOfficeBestPage} />
              </div>
            </div>
          </section>
        </>
