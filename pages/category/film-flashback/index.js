...
export async function getStaticProps() {
  ...
  let FilmFlashbackData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/film-flashback/&page_no=' + 1 + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const FilmFlashback = ({ data, FilmFlashbackData }) => {
  ...
  const loadFilmFlashbackData = () => {
    axios
      .get(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/film-flashback/&page_no=' + FilmFlashbackPage + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
      .then((res) => {
        ...
        <section className='sidebar_blockarea'>
          <div className='container'>
            <div className='sidebar_blockiner df fww'>
              ...
              <Info data={FilmFlashbackListData} setCurrentPage={setCurrentPage} totalPages={FilmFlashbackPages} requestFrom='FilmFlashback' currPage={FilmFlashbackPage} />
            </div>
          </div>
        </section>
      </>
