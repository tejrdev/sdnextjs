...
export async function getStaticProps() {
  ...
  let BoxOfficeOutlookData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/box-office-outlook/&page_no=' + 1 + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const BoxOfficeOutlook = ({ data, BoxOfficeOutlookData }) => {
  ...
  const loadBoxOfficeOutlookData = () => {
    axios
      .get(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/box-office-outlook/&page_no=' + BoxOfficeOutlookPage + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
      .then((res) => {
        ...
        <section className='sidebar_blockarea'>
          <div className='container'>
            <div className='sidebar_blockiner df fww'>
              ...
              <Info data={BoxOfficeOutlookListData} setCurrentPage={setCurrentPage} totalPages={BoxOfficeOutlookPages} currPage={BoxOfficeOutlookPage} />
            </div>
          </div>
        </section>
      </>
