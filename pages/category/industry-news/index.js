...
export async function getStaticProps() {
  ...
  let IndustryNewsData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/industry-news/&page_no=' + 1 + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const IndustryNews = ({ data, IndustryNewsData }) => {
  ...
  const loadIndustryNewsData = () => {
    axios
      .get(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/industry-news/&page_no=' + IndustryNewsPage + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
      .then((res) => {
        ...
        <section className='sidebar_blockarea'>
          <div className='container'>
            <div className='sidebar_blockiner df fww'>
              ...
              <Info data={IndustryNewsListData} setCurrentPage={setCurrentPage} totalPages={IndustryNewsPages} currPage={IndustryNewsPage} />
            </div>
          </div>
        </section>
      </>
