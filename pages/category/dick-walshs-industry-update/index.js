...
export async function getStaticProps() {
  ...
  let IndustryUpdateData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/dick-walshs-industry-update/&page_no=' + 1 + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const IndustryUpdate = ({ data, IndustryUpdateData }) => {
  ...
  const loadIndustryUpdateData = () => {
    axios
      .get(process.env.NEXT_PUBLIC_SD_API + '/news_page/news_category.php?url=' + process.env.NEXT_PUBLIC_MENU_URL + 'category/dick-walshs-industry-update/&page_no=' + IndustryUpdatePage + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN)
      .then((res) => {
        ...
        <section className='sidebar_blockarea'>
          <div className='container'>
            <div className='sidebar_blockiner df fww'>
              ...
              <Info data={IndustryUpdateListData} setCurrentPage={setCurrentPage} totalPages={IndustryUpdatePages} currPage={IndustryUpdatePage} />
            </div>
          </div>
        </section>
      </>
