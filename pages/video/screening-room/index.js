...
export async function getStaticProps() {
  ...
  const res = await fetch(process.env.NEXT_PUBLIC_SEO_LINK + 'video/screening-room');
  const data = await res.json();

  // screening room videos page static data
  let BoxofficeBuzzData = await fetch(
   process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + 1 + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/screening-room/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN
  );
...
const ScreeningRoom = ({ data , BoxofficeBuzzData }) => {
...
//console.log(BoxofficeBuzzData);
  ...
  const loadBoxofficeBuzzData = () => {
    axios
      .get(
        process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + distributorPage + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/screening-room/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN
      )
      .then((res) => {
