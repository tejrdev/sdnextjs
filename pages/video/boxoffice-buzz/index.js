...
export async function getStaticProps() {
  ...
  const res = await fetch(process.env.NEXT_PUBLIC_SEO_LINK + 'video/boxoffice-buzz');
  const data = await res.json();

  // box office videos page static data
  let BoxofficeBuzzData = await fetch(
    process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + 1 + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/boxoffice-buzz/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN
  );
...
const BoxofficeBuzz = ({ data , BoxofficeBuzzData }) => {
  ...
  const loadBoxofficeBuzzData = () => {
    axios
      .get(
        process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + distributorPage + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/boxoffice-buzz/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN
      )
      .then((res) => {
        // if (distributorPage === 1) {
        //   setBoxofficeBuzzData(res.data);
           setBoxofficeBuzzDataLoaded(true);
        //   console.log(process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + distributorPage + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/boxoffice-buzz/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
        // }
