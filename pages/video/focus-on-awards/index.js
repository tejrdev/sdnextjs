...
export async function getStaticProps() {
  ...
  const res = await fetch(process.env.NEXT_PUBLIC_SEO_LINK + 'video/focus-on-awards');
  const data = await res.json();

  // focus on awards videos page static data
  let BoxofficeBuzzData = await fetch(
    process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + 1 + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/focus-on-awards/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN
  );
...
const FocusOnAwards = ({ data , BoxofficeBuzzData }) => {
  ...
  const loadBoxofficeBuzzData = () => {
    axios
      .get(
        process.env.NEXT_PUBLIC_SD_API + '/video_page/video_sub_page.php?page=' + distributorPage + '&url=' + process.env.NEXT_PUBLIC_MENU_URL + 'video/focus-on-awards/?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN
      )
      .then((res) => {
