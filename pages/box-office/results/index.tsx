...
const BoxOfficeResults: React.FC<BoxOfficeResultsProps> = ({ BoxOfficeLoadedData }) => {
  const router = useRouter();
  const { yyyy, ww, tab } = router.query;
  // State management
  ...
  const [currentyear, setCurrentyear] = useState<number>(() => {
    // Initialize with URL parameter if available, otherwise use default
    return yyyy ? parseInt(yyyy as string) : parseInt(boxOfficeResultData.year);
  });
  const [currentWeek, setCurrentWeek] = useState<number>(() => {
    // Initialize with URL parameter if available, otherwise use default
    return ww ? parseInt(ww as string) : parseInt(boxOfficeResultData.week);
  });
  ...
  const meta_title = `Box Office Results: Weekend & Weekly Performance | Screendollars Box Office`;
  ...
  const canonicalUrl = process.env.NEXT_PUBLIC_FRONTEND_URL + '/box-office/results/';
  ...
  useEffect(() => {
    if (yyyy && ww && yyyy !== '' && ww !== '') {
      const year = parseInt(yyyy as string);
      const week = parseInt(ww as string);
      setCurrentyear(year);
      setCurrentWeek(week);
  ...
  useEffect(() => {
    if (toggleon) {
      router.push(router.pathname + '?tab=weekly');
    } else {
      router.push(router.pathname + '?tab=weekend');
    }
  }, [toggleon]);
  ...
  return (
    <>
      <HeadComponent jsonSchema={generateJsonLd()} meta_title={meta_title} meta_description={meta_description} canonical_url={canonicalUrl} />
      {/* <BoxOfficeResultsNavigation /> */}
      <div className={'boxofresult'}>
        <Boxofficetitle
          title={toggleon ? 'Box Office Results Weekly' : 'Box Office Results Weekend'}
          description={toggleon ? 'Domestic Box Office Results (7-Day Friday-Thursday)' : 'Domestic Box Office Results (3-Day Friday-Sunday)'}
        />
