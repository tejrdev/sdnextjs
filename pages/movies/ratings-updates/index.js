...
export async function getStaticProps() {
  ...
  const res = await fetch(process.env.NEXT_PUBLIC_SEO_LINK + 'movies/ratings-updates');
  const data = await res.json();

  //RatingUpdatesData static data
  let RatingUpdatesData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/film_data_pages/ratings_updates.php?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
