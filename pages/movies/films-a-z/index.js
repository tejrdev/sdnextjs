...
export async function getStaticProps() {
  ...
  const res = await fetch(process.env.NEXT_PUBLIC_SEO_LINK + 'movies/films-a-z');
  const data = await res.json();

  // film a-z static data
  let FilmsData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/film_data_pages/films_a_z.php?filter=' + 'a' + '&page_no=' + 1 + '&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const Films = ({ data, FilmsData }) => {
  ...
  useEffect(() => {
    ...
    $('#film-datatable').DataTable({
      columns: [{ data: 'title' }, { data: 'distributor' }, { data: 'release_date' }, { data: 'rating' }, { data: 'runtime' }, { data: 'dist_type' }, { data: 'dist_pattern' }, { data: 'dist_locations' }, { data: 'box_office_opening_weekend' }, { data: 'box_office_total' }],
                      ...
                      <table className='responsive dataTable twotblhead' id='film-datatable'>
                        <thead>
                          <tr>
                            ...
                            <th colSpan='1'></th>
                            <th colSpan='2'></th>
                            <th colSpan='2' data-title='Distribution'>
                              Distribution
                            </th>
                          ...
                          <tr>
                            ...
                            <th data-title='Runtime'> Locations <br /> +-LW </th>
