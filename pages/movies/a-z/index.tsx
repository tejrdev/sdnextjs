...
import { MoviesAZProps, MoviesAZResponse, MovieSearchResponse, DistributorSearchResponse, MovieListItem } from '@/types/movies';

// const APIEndpoint = 'https://live.screendollars.com/sd_api/';

export const getStaticProps: GetStaticProps<MoviesAZProps> = async () => {
  try {
    ...
    const FilmsDataResponse = await fetch(process.env.NEXT_PUBLIC_SD_API + '/film_data_pages/movie-a-z.php?&page_no=1&api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const Films: React.FC<MoviesAZProps> = ({ data, FilmsData }) => {
  ...
  const [FilmsYear, setFilmsYear] = useState<string>('All Years');
  ...
  useEffect(() => {
    loadFilmsData();
  }, [FilmsPage, FilmsYear, FilmsDistributor, FilmsSearchFilter]);
  ...
  const loadFilmsData = useCallback(async () => {
    try {
      ...
      let API_URL = `${process.env.NEXT_PUBLIC_SD_API}/film_data_pages/movie-a-z.php?page_no=${FilmsPage}&api_token=${process.env.NEXT_PUBLIC_API_TOKEN}`;

      if (FilmsYear !== 'All Years') {
  ...
  // Data arrays
  const alphanumericArray1 = [...Array.from({ length: 26 }, (_, i) => String.fromCharCode(97 + i)), ...Array.from({ length: 10 }, (_, i) => i.toString())];

  let yeararray = [...Array.from({ length: 9 }, (_, i) => (2026 - i).toString())];
  yeararray.push('2017-2000', '1999-1900');
  ...
  return (
    <>
      <HeadComponent data={data} />
      {/* <CategoryNavigation /> */}
      <section className='filmaz subfilmy alldist_list'>
        <div className='container container2'>
          <div className='info_block'>
            <div className='info_box printarea wtbg'>
              ...
              <div className='info_txt printbox'>
                <div className='filmaztable tablebox flex flex-wrap gap-5'>
                  <div className='tbledftside w-64 sticky top-0 lg:relative z-10 flex items-start tab_items'>
                    <div
                      className={`bg-white lg:pt-0 lg:border-0 rounded-md overflow-y-auto lg:max-h-max max-h-screen absolute lg:relative transition-all duration-500 ${filtertoggle ? 'active p-2 border border-gray-200 w-full' : 'w-0 lg:w-auto overflow-hidden p-0'
                        }`}>
                      <div className='top_head flex items-center justify-between'>
                        <h5 className=''>Filter By Alphabet</h5>
                        <p className='mb-2 mr-4 cursor-pointer' onClick={() => router.push('/movies/a-z')}>
                          Clear
                        </p>
                      </div>
                      <div className='filmaz_filter min-w-[238px]'>
                        <ul className='df fww'>
                          {alphanumericArray1.map((item, index) => (
                            <li className={`numa ${loading || routeLoading ? 'disabled' : ''}`} key={index}>
                      ...
                      <div className='moviesbysrch '>
                        ...
                        <div className='film_search_box'>
                          <input
                            ...
                            onKeyPress={onSearchFilmKeyPress}
                            disabled={loading || routeLoading}
                          />
                      ...
                      <div className='top_head flex items-center justify-between mt-5'>
                        ...
                        <p className='mb-2 mr-4 cursor-pointer' onClick={() => setFilmsYear('All Years')}>
                          Clear
                        </p>
                      </div>
                      <div className='yearfilters'>
                        <FilmFilter data={yeararray} tag='years' setSelectedValue={setSelectedValue} currentValue={FilmsYear} mobsize disabled={loading || routeLoading} />
                      </div>
                      <div className='top_head flex items-center justify-between'>
                    ...
                    <div
                      ...
                      {filtertoggle ? (
                        ...
                        <div className='flex'>
                          <FaFilter className='mt-1 text-sm' /> <strong className='pl-2'>Filter</strong>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {(loading || routeLoading) && (
                    <div className='datatable_wrap w-full lg:w-[calc(100%-17.5rem)]'>
                  ...
                  {!loading && !routeLoading && FilmsListData && FilmsListData.length > 0 && (
                    <div className='datatable_wrap w-full lg:w-[calc(100%-17.5rem)]'>
                      <p className='text-sm text-gray-500 mb-2'>
                        Showing {(FilmsPage - 1) * 50 + 1} to {FilmsTotalMovies > FilmsPage * 50 ? FilmsPage * 50 : FilmsTotalMovies} of {FilmsTotalMovies} results
                      </p>
                      <table className='responsive dataTable w-full' id='film-datatable'>
                        <thead>
                          <tr>
                            ...
                            <th data-title='Release year' className='text-center'>
                              Release year
                            </th>
                            <th data-title='Release date' className='text-center'>
                              Release date
                            </th>
                            <th data-title='Rating' className='text-center'>
                              Rating
                            </th>
                            <th data-title='Distribution Pattern' className='text-center'>
                              Pattern
                            </th>
                            <th data-title='Box Office Total' className='text-center'>
                              Box Office Total
                            </th>
                          </tr>
                        </thead>
                      ...
                      <div className='text-center'><HomePageAds cls='ads_970 mt-7' format='horizontal' /></div>
                    </div>
                  )}
                  ...
                  {!loading && !routeLoading && FilmsListData && FilmsListData.length === 0 && (
                    <div className='datatable_wrap w-full lg:w-[calc(100%-17.5rem)]'>
                      <p>
                        No data found for filter applied{' '}
                        {FilmsYear && (
                          <>
                            {' '}
                            Filter By Year: <strong>{FilmsYear}</strong>
                          </>
                        )}{' '}
                        {FilmsDistributor && (
                          <>
                            {' '}
                            Filter By Distributor: <strong>{SearchDistributor || FilmsDistributorName}</strong>
                          </>
                        )}{' '}
                        {FilmsSearchFilter && (
                          <>
                            {' '}
                            Filter By Title: <strong>{FilmsSearchFilter}</strong>
                          </>
                        )}
                      </p>
                    </div>
      ...
      <Pagination totalPages={FilmsPages} setCurrentPage={setCurrentPage} requestFrom='films' currentPage={FilmsPage} />
    </>
  );
};
