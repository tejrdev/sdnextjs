...
export async function getStaticProps() {
  ...
  let ReleaseChangesData = await fetch(process.env.NEXT_PUBLIC_SD_API + '/film_data_pages/release_changes.php?api_token=' + process.env.NEXT_PUBLIC_API_TOKEN);
...
const ReleaseChanges = ({ data, ReleaseChangesData }) => {
  ...
  useEffect(() => {
    ...
    /* $('#sr-datatable').DataTable({
      columns: [{ data: 'title' }, { data: 'distributor' }, { data: 'release_date' }, { data: 'change_comment' }],
      paging: false,
      searching: false,
      responsive: false,
      autoWidth: false,
